...
class PongEnv:
    def __init__(self, render_mode=None, target_fps = 60):
        ...
        self.paddle_width = 0.26  # 패들 너비 (화면 대비 비율)
        ...
        # 공과 패들 속도 설정
        self.ball_speed_min = 0.005  # 시작 속도
        self.ball_speed_max = 0.025  # 최대 속도
    ...
    def step(self, action):
        ...
        info : 추가 정보 (딕셔너리)
        """
        # 점수에 따라 속도 증가 (최대 80점까지 선형 증가, 증가폭 완만하게)
        speed_ratio = min(self.score / 80, 1.0)  # 80점까지 선형 증가
        ...
        # 빨간 공 등장 로직 (15점부터 5점 간격)
        if self.score >= self.red_ball_start_score:
            # 등장해야 할 빨간공 개수
            num_red_balls = (self.score - self.red_ball_start_score) // self.red_ball_interval + 1
            while len(self.red_balls) < num_red_balls:
                # x 위치 랜덤, y=0에서 시작
                ball_x = np.random.uniform(0.1, 0.9)
                self.red_balls.append({'x': ball_x, 'y': 0.0, 'active': True})
        # 빨간 공 이동
        for ball in self.red_balls:
            if ball['active']:
                ball['y'] += self.red_ball_speed
                # 패들 충돌 체크
                if 0.95 <= ball['y'] <= 1.0:
                    if abs(ball['x'] - self.paddle_x) <= self.paddle_width / 2:
                        done = True
                # 화면 아래로 벗어나면 비활성화
                if ball['y'] > 1.0:
                    ball['active'] = False
